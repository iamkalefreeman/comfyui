...
if (config.warmupCkpt) {
  ...
  checkpoint = checkpoint.default("flux1-dev-kontext_fp8_scaled.safetensors");
}
...
if (config.warmupCkpt) {
} else if (config.models.vae.enum.options.includes("ae.safetensors")) {
...
const RequestSchema = z.object({
  ...
  image_1: z
    .string()
    .describe("First image to stitch (URL or base64)"),
  image_2: z
    .string()
    .describe("Second image to stitch (URL or base64)"),
  stitch_direction: z
  ...
  feathering: z
    ...
    .optional()
    .default(0)
    .describe("The amount of feathering to apply at the seam"),
  seed: z
    .number()
    .int()
    .optional()
...
const workflow: Workflow = {
  ...
  description:
    "Edits and combines two input images based on a text prompt using the FLUX.1 Kontext model. This workflow stitches two images, then uses them as a reference for the text-guided generation.",
};
